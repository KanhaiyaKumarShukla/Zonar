...
class FirestoreService @Inject constructor(
    ...
    fun insertUser(firstName:String, lastName:String, role:String, email:String) {
        ...
        try {
            profileCollection.document(user.uid).set(user)
                .addOnSuccessListener { Log.d("FireStoreService", "New user added Successfully, $user") }
                .addOnFailureListener { Log.d("FireStoreService", "New user added Failed") }
        } catch (e: Exception) {
            // handel Exception
            Log.d("FireStoreService", e.message.toString())
        }
    }
